...
Eight ways QA charts misread. Each is a library or tool default, not a
malicious choice — which is why each needs an explicit rule. For every rule:
what happens, how to detect it in a chart or in chart code, and the fix.
...
against tick labels. Amplification here ≈ 50×.
...
question is never "what is the average test like" but "which tests changed".
...
where there is none. Most tools bridge nulls by default.
...
"Failed". Red/green conventions are hard-wired in readers, and ~8% of men
cannot separate red from green unless hues are chosen for separability.
...
the name. Fastest fix in the taxonomy: one `sort()` call.

## The metric: lie factor (LIE-01)

Tufte: lie factor = (% change shown visually) ÷ (% change in the data).
Honest ≈ 1; acceptable 0.8–1.2. A truncated bar pair showing a 650% visual
effect for a 1.4% data change has lie factor ≈ 46 — a number you can put in
a ticket. Compute it literally in review mode whenever A-1 or A-2 fails.
